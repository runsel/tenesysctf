...
$query = mysql_query("SELECT * FROM mission WHERE id_soal='$idsoal' and flag='$jawaban' ");
$hitung = mysql_num_rows($query);
}
if ($hitung > 0) {

	$datasoal = mysql_fetch_array($query);
	$kueri = mysql_query("SELECT * FROM player where id_player='$iduser' ");
	$datauser = mysql_fetch_array($kueri);
	...
	for ($i=1; $i<$ukuran; $i++) {
		...
		if ($datasoal['display'] == 0) {
			...
			mysql_query("UPDATE player set solved='$idsolve' , poin='$pointotal' , waktu=now() where id_player='$iduser' ") or die ("Update gagal, ".mysql_error());
			mysql_query("INSERT into lastsolved VALUES('','".$datauser['username']."','".$datasoal['id_soal']."','".$datasoal['nama_soal']."','".$datasoal['kategori']."','".$datasoal['level']."','".$waktu."')") or die ("Update gagal, ".mysql_error());
