...
if(!empty($_POST['id'])){
  $id = filter($_POST['id']);
  $lihat = mysql_query("SELECT * from mission where id_soal='$id' ");
              ...
              <div class="modal-dialog">
                <div class="modal-content p-0">
                  ...
                  <div class="tab-content"> 
                    ...
                    <ul>
                    <?php
                      $cek = mysql_query("SELECT * from lastsolved where id_soal='$id' limit 10");
                      $hitung = mysql_num_rows($cek);           
                      if ($hitung < 1) {
                        echo "<p>Belum ada player yang menyelesaikan misi ini.</p>";
                      }else{
                      $n= 1;
                      while ($dtsolve = mysql_fetch_array($cek)) {
                      $nama = mysql_query("SELECT nama_lengkap from player where username='$dtsolve[username]' ");
                      $l = mysql_fetch_array($nama);
                    ?>
